...
public class ChatServer extends UnicastRemoteObject implements InterfaceServer{
    ...
    //Gửi tin nhắn từ server đến tất cả hoặc một số client đã định danh bởi danh sách list
    @Override
    ...
    @Override
    public synchronized void logout(String clientName) throws RemoteException {
        for (int i = 0; i < clients.size(); i++) {
            try {
                if (clients.get(i).getName().equals(clientName)) {
                    clients.get(i).closeChat("You have logged out."); // Gửi thông báo đến client đã đăng xuất
                    clients.remove(i); // Xóa client khỏi danh sách
                    break; // Thoát vòng lặp sau khi tìm thấy client cần đăng xuất
                }
            } catch (RemoteException ex) {
                System.out.println("Error: " + ex.getMessage());
            }
